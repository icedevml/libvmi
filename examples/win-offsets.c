...
{
    fprintf(stderr, "Usage: %s name|domid <domain name|domain id> -r <rekall profile> [-v] [-k]\n", arg0);
    fprintf(stderr, "    [-v]   optional, enable verbose mode\n");
    fprintf(stderr, "    [-k]   optional, only print KPGD value\n");
}
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt (argc, argv, "kvr:")) != -1)
        switch (c) {
            case 'k':
                only_output_kpgd = 1;
                break;
    ...
    if (strcmp(argv[optind],"name")==0) {
        ...
        fprintf(stderr, "You have to specify either name or domid!\n");
        show_usage(argv[0]);
        return 1;
    }

    if (!rekall_profile) {
        fprintf(stderr, "You have to specify path to rekall profile!\n");
        show_usage(argv[0]);
        return 1;
    }
